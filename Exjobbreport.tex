...
field even after an external electric field is removed. The ferroelectric nature
of the material is not uniform but rather comprised of many small-scale
fields that can be aligned from an external source in order to propagate and
strengthen the field throughout the material. In the case of ferroelectric
materials the small-scale fields are referred to as domains where each domain
has its own electric field strength and direction in 3D-space. The domains are
separated by domain walls of varying thickness where the field is rapidly and
repeatedly changing between the domain field directions. The domain boundaries
can arise from electrical and thermal treatment of the samples but are mostly
determined by the spontaneously oriented fields of the domains 
\cite{damjanovic2006hysteresis}.
...
\begin{figure}[htbp]
    ...
    \caption{Schematic figure of the process of domain alignment ending with a
    polarized sample without an external source.}
...
\begin{figure}[htbp]
    ...
    \caption{2D energy landscape of the polarization of a ferroelectric
    material. Maybe taken from R. Athle \cite{athle2019development}?}
    \label{fig:theo_Ec}
\end{figure}
...
polarization of a sample with the electric field strength applied to the
sample. For a non-ferroelectric material the P-E curve would show a linear
relationship between the two. However, for a ferroelectric material clear
...
\ref{fig:theo_PE}. 
% TODO: Describe the figure in detail here!
% Important note: The absolute value of P is not measured. Only the change of
% it.
...
% de olika kristallstrukturerna (faser) här och hur orthorhombic inte är
% centrocsymmetric. Grain boundaries?!
\cite{migita2019phase}

\subsection{Capping Using \ce{TiN}}

% Mål: Få med att capping suppressar den monocliniska fasen och att det är najs.
\cite{muller2012ferroelectricity}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{fig/PUND.png}
...
breakdown. %TODO: What causes additional leakage with cycling?
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\textwidth]{fig/CV_hysteresis.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{fig/UniCV.png}
